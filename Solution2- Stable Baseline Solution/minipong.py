...
class Minipong(gym.Env):
    ...
    def __init__(self, level = 5, size = 5, normalise = True):
        ...
        self.observation_space = spaces.Box(
          low=np.array([-1,-3,-1]), high=np.array([1,3,1]), dtype=np.float16)
        
        self.action_space = spaces.Discrete(3)
        self.running_reward_list_per_episode = []
        self.episode_reward_sum = 0
        self.episode_wise_reward_sum = []
        self.running_reward_episode =  0
        self.transition()
    ...
    def terminal(self):
        ...
        if y==1:
            #this means episode has finished
            self.running_reward_episode = 0.05 * self.episode_reward_sum + (1 - 0.05) * self.running_reward_episode
            self.running_reward_list_per_episode.append(self.running_reward_episode)
            self.episode_wise_reward_sum.append(self.episode_reward_sum)
            self.episode_reward_sum = 0
            return True
        else:
            return False
    ...
    def reward(self, n, p, action):
        ...
        if self.level == 3:
            ...
            if (p[1] < 0 and action == 1) or (p[1] > 0 and action == 2):
                r+=8
                
            r +=2                 # for every step i am giving constant reward so that agent will get the idea that it has to play as long as it can
    ...
    def step(self, action):
        ...
        r = self.reward(next_state, previous_state, action)
        done = self.terminal()
